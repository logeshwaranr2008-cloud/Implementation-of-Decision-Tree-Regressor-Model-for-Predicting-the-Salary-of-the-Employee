...
#Ex 09 - Implementation of Decision Tree Regressor Model for Predicting the Salary of the 
Employee
# Import libraries
import pandas as pd
import numpy as np
...
# ------------------------------
X_grid = np.arange(min(X.values), max(X.values)+0.01, 0.01)  # High-resolution for 
smoother curve
...
plt.xlabel('Level')
Thus the program to implement the Decision Tree Regressor Model for Predicting the Salary of the
Employee is written and verified using python programming.
plt.ylabel('Salary')
plt.legend()
plt.show()
```
...
## Output:
<img width="862" height="657" alt="image" src="https://github.com/user-attachments/assets/74f0e538-04f8-4ae4-b8c9-ed51dbec1ee0" />
